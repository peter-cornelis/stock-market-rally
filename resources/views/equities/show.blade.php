<x-layout>
    ...
    <section class="max-w-4xl mx-auto mt-8 md:px-4">
        ...
        <div class="max-w-3xl mx-auto mt-10 px-4 md:py-6 py-4 bg-white md:rounded-lg shadow">
            ...
            <canvas id="myChart" height="120" data-chart-data="{{ json_encode($equity->charts) }}" data-symbol="{{ $equity->symbol }}"></canvas>
            @auth
            <nav class="flex gap-2 mt-4 justify-center">
                ...
                <ul class="grid grid-flow-col gap-6">
                    <li class="text-center">
                        <a href="/transactions/create/{{ $equity->id }}?type=buy" class="inline-block w-32 text-sm text-center bg-blueFin hover:bg-blueFin/90 text-white font-bold rounded border border-black/20 shadow px-3 py-1.5 mt-4">Koop</a>
                    </li>
                    @if (auth()->user()->equities->contains($equity->id))
                    <li class="text-center">
                        <a href="/transactions/create/{{ $equity->id }}?type=sell" class="inline-block w-32 text-sm text-center bg-error hover:bg-error/90 text-white font-bold rounded border border-black/20 shadow px-3 py-1.5 mt-4">Verkoop</a>
                    </li>
                    @endif
                </ul>
            @endauth
        ...
        <article class="grid sm:grid-cols-[auto_1fr_1fr_1fr] text-center max-w-3xl mx-auto mt-10 px-6 py-4 bg-white md:rounded-lg shadow">
            <div class="text-left max-md:pb-4 sm:pr-10 max-md:mx-auto">
                <h3 class="text-lg font-bold text-black/50 mb-2">Financiele ratio's</h3>
                <span class="block">Beta waarde: {{ $equity->financialRatio->beta }}</span>
                <span class="block">Omzet per aandeel: {{ $equity->financialRatio->revenuePerShare }}</span>
                <span>dividendrendement: {{ $equity->financialRatio->dividendYieldPercentage }} %</span>

            </div>
            <div class="relative max-sm:border-t sm:border-l border-black/20 my-auto max-sm:py-4">
            ...
            <div class="relative max-sm:border-t sm:border-l border-black/20 my-auto max-sm:pt-4">
                <h3>Current ratio</h3>
