...
<li class="mt-4 p-1 first:mt-8 bg-white md:rounded-lg shadow">
    <a href="/equities/{{ $equity->id }}" class="grid grid-cols-[auto_1fr_auto] gap-6 max-w-4xl py-2 px-4 md:rounded-lg hover:bg-black/2">
        <img src="{{ $equity->company->image }}" alt="" class="w-16 h-16 p-1 my-auto rounded shadow bg-black/80"/>
        <div>
            <h3 class="text-lg font-semibold">{{ $equity->company->name }}<span class="py-2 text-sm max-sm:hidden"> | <span class="text-black/60 py-2">{{ $equity->symbol }}</span> | {{ $equity->exchange->name }}</span></h3>   
            <p class="text-sm max-md:hidden">
